package com.fouo.tx.day12;

import java.util.ArrayList;

/**
 * @author KONGLH
 * @title: 整棵树找到是搜索二叉树且最大的子树，返回其节点数。
 * @projectName
 * @description:
...
//	}
    ...
    private static Info process(Node x) {
        ...
        if (leftBST && rightBST) {
            boolean leftLess = leftInfo == null ? true : (leftInfo.max < x.value);
            boolean rightMore = rightInfo == null ? true : (rightInfo.min > x.value);
            ...
            if (leftLess && rightMore) {
                ...
                p3 = Math.max(p3,(1+leftSize+rightSize));
            }
        }
        int maxSubBSTSize = Math.max(p1,Math.max(p2,p3));
        return new Info(maxSubBSTSize,min,max,allSize);
    }
    ...
    public static class Info {
        ...
        public Info(int maxSubBSTSize, int min, int max, int allSize) {
